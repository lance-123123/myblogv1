...
function GetData() {
    ...
    resultDataTable = $searchResult.dataTable({
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex) {
            ...
            if (aData.status == 1) {
                $(nRow).find("td:eq(1)").html("<a class='btn btn-danger btn-xs' data='" + aData.id + "," + aData.status + "' onclick='change(this,event)'><i class='glyphicon glyphicon-remove'></i> 禁用 </a>");
            } else if (aData.status == 0) {
                $(nRow).find("td:eq(1)").html("<a class='btn btn-info btn-xs' data='" + aData.id + "," + aData.status + "' onclick='change(this,event)'><i class='glyphicon glyphicon-ok'></i> 启用 </a>");
