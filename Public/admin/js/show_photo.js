...
function GetData() {
    ...
    resultDataTable = $searchResult.dataTable({
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex) {
           ...
           if (aData.status == 1) {
                $(nRow).find("td:eq(2)").html("<a class='btn btn-danger btn-xs' data='" + aData.id + "," + aData.status + "' onclick='change(this,event)'><i class='glyphicon glyphicon-remove'></i> 禁用 </a>");
            } else if (aData.status == 0) {
                $(nRow).find("td:eq(2)").html("<a class='btn btn-info btn-xs' data='" + aData.id + "," + aData.status + "' onclick='change(this,event)'><i class='glyphicon glyphicon-ok'></i> 启用 </a>");
...
function change(e, event) {//由启用状态到禁用状态
    ...
    var status = nums[1];
    if (status == 0) {
        bootConfirm("亲，您确定要禁用这张图片吗？", function () {
            change_close(id, status);
        });
    } else if (status == 1) {
        bootConfirm("亲，您确定要启用这张图片吗？", function () {
            change_close(id, status);
        });
    }
