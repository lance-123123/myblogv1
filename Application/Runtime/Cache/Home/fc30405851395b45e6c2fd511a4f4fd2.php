...
<input type="hidden" value="<?php echo U('Index/add_blog_number');?>" id="plug_number">
  ...
  <article>
    ...
    <h2 class="about_h">您现在的位置是：<a href="javascript:void()">文章</a><a href="javascript:void()"></a></h2>
    <div class="bloglist">
<input type="hidden" value="<?php echo U('Index/add_blog_number');?>" id="plug_number">
     <?php if(is_array($blog_list)): $i = 0; $__LIST__ = $blog_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="newblog">
        <ul>
          ...
          <p><?php echo (htmlspecialchars_decode($vo["blog_newsletter"])); ?><a href="<?php echo U('Index/look_blog_detail');?>?id=<?php echo ($vo["id"]); ?>" data="<?php echo ($vo["id"]); ?>" class="readmore">全文</a></p>
        </ul>
